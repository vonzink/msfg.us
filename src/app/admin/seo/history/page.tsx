...
export default async function SeoHistoryPage({
  ...
  const revisions = await listHistory(ctx.tenant.id, "PAGE_SEO", path);

  return (
    <div className="max-w-2xl">
      <h1 className="text-2xl font-extrabold">SEO history — {path}</h1>
            ...
            <tr key={r.id} className="border-b border-line">
              ...
              <td className="text-right">
                <form
                  action={async () => {
                    "use server";
                    await rollbackSeoAction(path, r.version);
                  }}
                >
